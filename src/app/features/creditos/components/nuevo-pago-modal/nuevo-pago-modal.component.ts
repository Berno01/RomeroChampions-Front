        ...
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white">
          <div>
            ...
            <p class="text-sm text-gray-500">
              @if (currentStep() === 'SELECT_DEUDOR') { Paso 1: Seleccionar Cliente }
              @else if (currentStep() === 'SELECT_DEUDA') { Paso 2: Seleccionar Deuda }
              @else { Paso 3: Confirmar Pago }
            </p>
          </div>
                 ...
                 @for (deudor of filteredDeudores(); track deudor.id_cliente) {
                   ...
                   >
                     <div class="flex justify-between items-start mb-2">
                       <h3 class="font-bold text-gray-900 group-hover:text-black">{{ deudor.nombre_completo }}</h3>
                     ...
                     <div class="pt-3 border-t border-gray-100 flex justify-between items-end">
                       ...
                       <span class="text-lg font-bold text-red-600">Bs. {{ deudor.total_deuda | number:'1.2-2' }}</span>
                     </div>
                   </div>
                 } @empty {
          ...
          @if (currentStep() === 'SELECT_DEUDA') {
            <div class="space-y-4">
               <div class="flex items-center gap-2 mb-4">
                 <button (click)="goBack()" class="text-sm text-gray-500 hover:text-black hover:underline flex items-center gap-1">
                   ← Volver a clientes
                 </button>
                 ...
                 @for (venta of deudasCliente(); track venta.id_venta) {
                   ...
                   >
                     <div>
                       ...
                       <p class="text-xs text-gray-500">Fecha: {{ venta.fecha_venta | date:'dd/MM/yyyy HH:mm' }}</p>
                       <p class="text-xs text-gray-500 mt-1">Total Original: <span class="font-medium">Bs. {{ venta.total_original | number:'1.2-2' }}</span> • Items: {{ venta.cantidad_items }}</p>
                     </div>
                     
                     <div class="text-right">
                       <span class="block text-xs text-gray-500 uppercase font-bold tracking-wider">Saldo Pendiente</span>
                       <span class="text-xl font-bold text-red-600">Bs. {{ venta.saldo_pendiente | number:'1.2-2' }}</span>
                     </div>
                   </div>
                 } @empty {
                   <div class="py-10 text-center text-gray-500 bg-white rounded-lg border border-dashed border-gray-300">
          ...
          @if (currentStep() === 'CONFIRM_PAGO') {
            <div class="max-w-md mx-auto space-y-6">
               <div class="text-center">
                 ...
                 <p class="text-sm text-gray-500">
                   Venta #{{ selectedVenta()?.id_venta }} • Cliente: {{ selectedDeudor()?.nombre_completo }}
                 </p>
               </div>
               ...
               <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-5">
                 <div class="flex justify-between items-center p-3 bg-red-50 rounded-lg border border-red-100">
                   ...
                   <span class="text-lg font-bold text-red-800">Bs. {{ selectedVenta()?.saldo_pendiente | number:'1.2-2' }}</span>
                 </div>
                 ...
                 <div class="space-y-4">
                   <!-- Monto -->
                   <div>
                     <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Monto a Pagar (Bs)</label>
                     <div class="relative">
                   ...
                   <!-- Observacion -->
                   <div>
                     <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Observación (Opcional)</label>
                     <textarea
                       [ngModel]="observacion()"
...
export class NuevoPagoModalComponent {
  ...
  montoError = computed(() => {
    ...
    if (monto > venta.saldo_pendiente) return `El monto excede el saldo pendiente (Bs. ${venta.saldo_pendiente}).`;
    return '';
  });
